...
<div class="card">
  ...
  <div class="body-inner">
    ...
    <table>
      ...
      ${vatInclusive
        ? `${row("Suggested price (VAT inclusive)", fmtGbp(result.totalPrice), true)}
      ${row("VAT", "Included in price (0%)")}
      <tr class="total-row">
        <td>Total price</td>
        <td style="text-align:right;font-family:monospace">${fmtGbp(result.totalPrice)}</td>
      </tr>`
    ...
    <p class="note">
      ...
      Print time used for electricity calculation: ${printHours.toFixed(2)} hours.${vatInclusive ? ' VAT is included in the quoted price.' : ''}
    </p>
  </div>
</div>
...
export default function AdminPricingPage() {
  ...
  const exportInvoice = () => {
    const html = generateInvoiceHtml(inputs, result, vatInclusive);
        ...
        <div className="space-y-4">
          <div className="bg-brand-surface border border-brand-border rounded-sm p-5 shadow-card sticky top-8">
            ...
            <div className="mt-4 pt-4 border-t-2 border-brand-border">
              {vatInclusive ? (
                <CostRow
                  label="Suggested Price (VAT inclusive)"
            ...
            <div className="mt-4 p-3 bg-brand-arctic rounded-sm">
              <label className="flex items-center gap-2 cursor-pointer mb-2">
                ...
                />
                <span className="text-xs text-brand-text font-medium">VAT inclusive pricing</span>
              </label>
              <p className="text-xs text-brand-muted">
                {vatInclusive
                  ? 'VAT is included in the price. Invoice will show 0% VAT.'
                  : 'Use the suggested price as a starting point. Adjust based on market demand, complexity, and perceived value.'}
              </p>
            </div>
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      const invoiceHtml = generateInvoiceHtml(inputs, result, vatInclusive);
