...
async function getBestSellingProducts(): Promise<ProductWithImages[]> {
  ...
  const productIds = bestSelling.map((b) => b.productId).filter(Boolean) as string[]

  const result = await db.query.products.findMany({
    where: sql`${products.id} IN (${sql.join(productIds.map(id => sql`${id}`), sql`, `)}) AND ${products.active} = true`,
    with: { images: { orderBy: asc(productImages.order) } },
  })
...
export default async function HomePage() {
  const [featuredProducts, latestProducts, bestSellingProducts, activeBundles] = await Promise.all([
    getFeaturedProducts(),
    getLatestProducts(),
  ...
  return (
    <>
      <HeroSection />
      <ValueProps />
      <FeaturedProducts featuredProducts={featuredProducts} displayProducts={displayProducts} />
      <ProcessSection />
