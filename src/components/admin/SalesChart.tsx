...
const CustomTooltip = ({ active, payload, label }: {
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-brand-surface border border-brand-border rounded-sm px-4 py-3 shadow-card-lg">
    ...
    <div className="bg-brand-surface border border-brand-border rounded-sm p-6 shadow-card">
      ...
      <div className="space-y-2">
        {data.map((product) => (
          <div
            key={product.id}
            className="flex items-center justify-between px-3 py-2 rounded-sm bg-red-50 border border-red-100 dark:bg-red-950/50 dark:border-red-900"
          >
            <span className="text-sm font-medium text-brand-text truncate pr-2">
              {product.name}
            </span>
